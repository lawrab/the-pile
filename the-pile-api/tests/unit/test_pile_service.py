...
class TestPileService:
    ...
    @pytest.mark.asyncio
    async def test_get_steam_owned_games_success(self, pile_service, mock_steam_owned_games):
    ...
    @pytest.mark.asyncio
    async def test_get_steam_app_details_success(self, pile_service, mock_steam_app_details):
    ...
    @pytest.mark.asyncio
    async def test_start_playing_success(self, pile_service, db_session, sample_pile_entry):
    ...
    @pytest.mark.asyncio
    async def test_mark_completed_success(self, pile_service, db_session, sample_pile_entry):
    ...
    @pytest.mark.asyncio
    async def test_mark_abandoned_success(self, pile_service, db_session, sample_pile_entry):
    ...
    @pytest.mark.asyncio
    async def test_update_status_all_statuses(self, pile_service, db_session, sample_pile_entry):
    ...
    @pytest.mark.asyncio
    async def test_update_status_invalid_status(self, pile_service, db_session, sample_pile_entry):
    ...
    @pytest.mark.asyncio
    async def test_get_user_pile_with_filters(self, pile_service, db_session, sample_user, sample_steam_game):
        ...
        for i, status in enumerate(statuses):
            game = SteamGame(
                steam_app_id=500 + i,  # Use different range to avoid conflicts with sample_steam_game (400)
    ...
    @pytest.mark.asyncio
    async def test_import_steam_library_new_games(self, pile_service, db_session, sample_user, 
                                                   mock_steam_owned_games, mock_steam_app_details):
        """Test importing Steam library with new games."""
        with patch.object(pile_service, 'get_steam_owned_games', return_value=mock_steam_owned_games["response"]["games"]), \
             patch.object(pile_service, 'get_steam_app_details', return_value=mock_steam_app_details["400"]["data"]):
            ...
            pile_entries = db_session.query(PileEntry).filter(PileEntry.user_id == sample_user.id).all()
    ...
    @pytest.mark.asyncio
    async def test_sync_playtime_updates_existing_entries(self, pile_service, db_session, 
                                                         sample_user, sample_pile_entry, 
        ...
        with patch.object(pile_service, 'get_steam_owned_games', return_value=modified_games):
