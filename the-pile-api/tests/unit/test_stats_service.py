...
class TestStatsService:
    ...
    def pile_with_varied_games(self, db_session, sample_user):
        ...
        games_data = [
            ...
            {"name": "Cyberpunk 2077", "price": 59.99, "playtime": 0, "status": GameStatus.UNPLAYED, "purchase_days_ago": 365, "app_id": 1000},
            # Playing game (medium shame)
            {"name": "The Witcher 3", "price": 39.99, "playtime": 1200, "status": GameStatus.PLAYING, "purchase_days_ago": 180, "app_id": 1001},
            # Completed game (no shame)
            {"name": "Portal 2", "price": 19.99, "playtime": 480, "status": GameStatus.COMPLETED, "purchase_days_ago": 90, "app_id": 1002},
            # Abandoned game (high shame)
            {"name": "Dark Souls III", "price": 49.99, "playtime": 30, "status": GameStatus.ABANDONED, "purchase_days_ago": 200, "app_id": 1003},
            # Amnesty granted (reduced shame)
            {"name": "Flight Simulator", "price": 69.99, "playtime": 5, "status": GameStatus.AMNESTY_GRANTED, "purchase_days_ago": 400, "app_id": 1004},
        ]
    ...
    @pytest.mark.asyncio
    async def test_calculate_basic_shame_score(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_shame_score_components(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_shame_score_time_penalty(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_shame_score_ranking(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_reality_check_calculations(self, stats_service, pile_with_varied_games, sample_user, db_session):
        """Test reality check engine calculations."""
        reality_check = await stats_service.calculate_reality_check(sample_user.id, db_session)
        ...
        assert reality_check.money_wasted > 0     # Should have money wasted on unplayed games
        assert reality_check.completion_years > 0 # Should take time to complete

    @pytest.mark.asyncio  
    async def test_reality_check_money_calculation(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_reality_check_most_expensive(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_progressive_shame_calculation(self, stats_service, db_session, sample_user):
        ...
        for game_count in [1, 5, 10]:
            ...
            score_result = await stats_service.calculate_shame_score(sample_user.id, db_session)
    ...
    async def test_edge_cases(self, stats_service, db_session, sample_user):
        ...
        empty_result = await stats_service.calculate_shame_score(sample_user.id, db_session)
    ...
    @pytest.mark.asyncio
    async def test_user_score_update(self, stats_service, pile_with_varied_games, sample_user, db_session):
    ...
    @pytest.mark.asyncio
    async def test_generate_insights(self, stats_service, pile_with_varied_games, sample_user, db_session):
