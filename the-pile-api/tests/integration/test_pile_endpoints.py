...
class TestPileEndpoints:
    ...
    def test_get_pile_with_entries(self, client, auth_headers, sample_pile_entry, mock_jwt_decode):
    ...
    def test_get_pile_with_status_filter(self, client, auth_headers, db_session, sample_user, sample_steam_game, mock_jwt_decode):
    ...
    def test_get_pile_with_pagination(self, client, auth_headers, db_session, sample_user, mock_jwt_decode):
    ...
    def test_import_steam_library_success(self, mock_import, client, auth_headers, mock_jwt_decode):
    ...
    def test_sync_playtime_success(self, mock_sync, client, auth_headers, mock_jwt_decode):
    ...
    def test_grant_amnesty_success(self, client, auth_headers, sample_pile_entry, db_session, mock_jwt_decode):
        """Test successful amnesty granting."""
        amnesty_data = {
            "reason": "Game is too difficult for my current skill level"
        }
        ...
        assert sample_pile_entry.amnesty_reason == "Game is too difficult for my current skill level"

    def test_grant_amnesty_nonexistent_game(self, client, auth_headers, mock_jwt_decode):
        """Test amnesty granting for non-existent game."""
        amnesty_data = {"reason": "Test reason"}
        
        response = client.post(
            "/api/v1/pile/amnesty/99999",
            json=amnesty_data,
            headers=auth_headers
        )
    ...
    def test_start_playing_success(self, client, auth_headers, sample_pile_entry, db_session, mock_jwt_decode):
    ...
    def test_mark_completed_success(self, client, auth_headers, sample_pile_entry, db_session, mock_jwt_decode):
    ...
    def test_mark_abandoned_success(self, client, auth_headers, sample_pile_entry, db_session, mock_jwt_decode):
    ...
    def test_update_status_direct_success(self, client, auth_headers, sample_pile_entry, db_session, mock_jwt_decode):
    ...
    def test_update_status_invalid_status(self, client, auth_headers, sample_pile_entry, mock_jwt_decode):
    ...
    def test_update_status_missing_status_field(self, client, auth_headers, sample_pile_entry, mock_jwt_decode):
        ...
        assert response.status_code == 400
        assert "Status field is required" in response.json()["detail"]  # Match actual API message
    ...
    def test_all_endpoints_require_auth(self, client, sample_pile_entry):
        ...
        for method, endpoint in endpoints:
            ...
            assert response.status_code == 403, f"Endpoint {method} {endpoint} should require auth"  # FastAPI returns 403

    def test_cross_user_access_protection(self, client, auth_headers, db_session, mock_jwt_decode):
