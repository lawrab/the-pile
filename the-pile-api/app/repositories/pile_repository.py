...
class PileRepository(BaseRepository[PileEntry]):
    ...
    def get_by_user_and_game(self, user_id: int, steam_game_id: int) -> Optional[PileEntry]:
    ...
    def get_playtime_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_playtime_hours": round(total_playtime / 60, 2) if total_playtime else 0,
    ...
    def update_status(self, user_id: int, steam_game_id: int, status: GameStatus, **kwargs) -> Optional[PileEntry]:
    ...
    def bulk_update_playtime(self, user_id: int, playtime_map: Dict[int, int]) -> int:
        ...
        entries = self.db.query(PileEntry).options(
            joinedload(PileEntry.steam_game)
        ).filter(PileEntry.user_id == user_id).all()
        ...
        for entry in entries:
            ...
            if app_id in playtime_map:
                ...
                if entry.playtime_minutes != new_playtime:
                    ...
                    if new_playtime == 0 and entry.status not in [GameStatus.AMNESTY_GRANTED, GameStatus.COMPLETED]:
    ...
    def count_by_status(self, user_id: int) -> Dict[GameStatus, int]:
        ...
        results = self.db.query(
            PileEntry.status,
            func.count(PileEntry.id).label('count')
        ).filter(
            PileEntry.user_id == user_id
        ).group_by(PileEntry.status).all()
