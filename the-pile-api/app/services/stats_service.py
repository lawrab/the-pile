...
class StatsService:
    ...
    async def calculate_shame_score(self, user_id: int, db: Session) -> ShameScore:
        ...
        zero_playtime_penalty = shame_data["zero_playtime_count"] * 3  # Extra penalty for never played
    ...
    async def generate_insights(self, user_id: int, db: Session) -> BehavioralInsights:
        ...
        if genre_analysis["most_neglected_genre"]:
            recommendations.append(f"Stop buying {genre_analysis['most_neglected_genre']} games until you play the ones you have")
        ...
        if financial_analysis["unplayed_value"] > 100:
            recommendations.append(f"You have ${financial_analysis['unplayed_value']:.0f} worth of unplayed games. That's a nice vacation!")
